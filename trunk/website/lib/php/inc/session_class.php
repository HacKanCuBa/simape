...
//        session_name($name);
        ...
        $vidautil = empty($lifetime) ? self::COOKIE_LIFETIME : $lifetime;

        // Setear cookie e iniciar sesion
        session_set_cookie_params($vidautil, $ruta, $dominio, $secure, true);
        
        $dontChangeID ?: session_regenerate_id(TRUE);
    ...
    /**
     * Fija como contraseña de sesión a la contraseña de sistema, que se basa 
     * en los token fijos y el ID de sesión, para emplear cuando el usuario 
     * aún no está autenticado.
     * 
     * @access public
    ...
    /**
     * Fija como contraseña de sesión al UID del usuario.
     * 
     * @access public
     * @tutorial https://code.google.com/p/simape/wiki/ModeloSeguridad
     ...
     */
    public function useUserPassword($UID)
    {
        if (Usuario::isValid_UID($UID)) {
            $this->setPasswordSalt(Crypto::getSystemPassword_salt());
            return $this->setPassword(Crypto::getHash($UID));
        }
