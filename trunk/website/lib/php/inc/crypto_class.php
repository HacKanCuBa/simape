...
{  
    ...
    public static function getSystemPassword_salt()
    {
        Session::retrieve(SMP_SESSINDEX_SYSTEMPASSWORDSALT) ?: 
            Session::store(SMP_SESSINDEX_SYSTEMPASSWORDSALT, static::getRandomIV());
    ...
    public static function encrypt($data, $password, $salt = NULL)
    {
        if (isset($data, $password)) {
            $passwd = self::makePasswdSalt($password, $salt);
            
            $iv = self::getRandomIV();
            
            $codedData = self::encodeData($data);
            if ($codedData) {
                $encString = self::encryptStr($codedData, $passwd, $iv);
                if ($encString) {
